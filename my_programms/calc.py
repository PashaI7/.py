...
def calculate(): #функция сalculate получает из списка stack операнды и операцию, которую над ними надо сделать
    global stack # 'global' позволяет изменять переменную за пределами текущей области. Это используется для создания глобальной переменной и внесения изменений в переменную в локальном контексте.
...
def click(text): #click обрабатывает нажатые клавиши(прослушка). Это определяется с помощью текста, который написан на самой кнопке, которую нажали
    global activestr
    global stack # 'stack' линейная структура данных, с помощью которой можно добавлять(.append) или удалять данные(.pop)
    if text == 'CE':
        stack.clear()
        activestr = ''
